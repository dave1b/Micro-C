...
tError quadParseCommand(const char *cmd)
{
	...
	{
		term_write_line("q (quad) commands:");
		term_write_line("  help");
		term_write_line("  status");
		term_write_line("  reset");
		result = EC_SUCCESS;
	}
	else if (strncmp(cmd, "status", sizeof("status")-1) == 0)
	{
		term_write_line("quad status:");
		term_write("L:");
		term_write_num16s(ticksLeft);
		term_write(" ");
		term_write_num16s(quadGetDistanceLeft());
		term_write(" ");
		term_write_num16s(quadGetSpeedLeft());
		term_write_line("");

		term_write("R:");
		term_write_num16s(ticksRight);
		term_write(" ");
		term_write_num16s(quadGetDistanceRight());
		term_write(" ");
		term_write_num16s(quadGetSpeedRight());
		term_write_line("");
		result = EC_SUCCESS;
	}
...
void quad_init(void)
{
  ...
  term_register_command_line_handler(&clh, "q", "(quad)", quadParseCommand);
}
