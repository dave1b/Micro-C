...
void sound_beep(uint16_t frequency, uint16_t timeMS)
{
  ...
  {
    // calculate the number of ticks needed to generate the desired frequency.
    ...
    // configure the channel status and control register as follows:
  ...
  // add to the current timer counter register the value of tmrFrequencyTicks calculated above and
...
{
  // muxing of PTC1 to FTM3_CH0!
  // configured over MCU ConfigTool
  PORTC->PCR[1] = PORT_PCR_MUX(4); // p220
