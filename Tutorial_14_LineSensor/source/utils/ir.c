...
void FTM0CH5_IRQHandler(void)
{
  ...
  if (GPIOD->PDIR & (1<<5))
  {        // Pin is High now => rising edge => pulse: --___--
    pulse = value - oldTicks;       // calc pule time
  }
  else
  {                            // Pin is Low now => falling edge => pause: __---__
    pause = value - oldTicks;       // calc pause time
    ir_process_receive_state_machine(pulse, pause);
  }
...
void ir_init(void)
{
	// is configured with config tool
}
